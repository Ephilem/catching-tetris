...
void Lcd_StartReadWriteGRAM(const Lcd_StartRWPacket *packet) {
    ...
    if (packet->size.x == 0 || packet->size.y == 0) {
        ...
        x0 = packet->pos.x;
        y0 = packet->pos.y;
        x1 = packet->pos.x + packet->size.x - 1;
        y1 = packet->pos.y + packet->size.y - 1;
    }

    if (deviceid == 0x8989) {
        write_reg(0x0044, (x1 << 8) | x0);
        write_reg(0x0045, y0);
        write_reg(0x0046, y1);
        // base 0x6070, AM = 3
        write_reg(0x0011, 0x6070 | ((packet->dir & 1) << 3));
    } else {
        write_reg(0x50, x0);
        write_reg(0x51, x1);
        write_reg(0x52, y0);
        write_reg(0x53, y1);
        //              BGR      I/D1    I/D0    AM (direction)
        write_reg(0x03, (1<<12)|(1<<5)|(1<<4)|((packet->dir & 1) << 3));
    }
    Lcd_SetCursor(x0, y0);
...
void Lcd_WriteFillGRAM(unsigned short data, unsigned int n) {
    LCD_CS(0);
    LCD_RS(1);
    while (n != 0) {
        lcd_send(data);
...
void Lcd_WriteBufferGRAM(const unsigned short *data, unsigned int n) {
    LCD_CS(0);
    LCD_RS(1);
    while (n != 0) {
        lcd_send(*data++);
...
void Lcd_Clear(unsigned short Color)
{
    ...
    Lcd_WriteFillGRAM(Color, LCD_WIDTH*LCD_HEIGHT);
}
